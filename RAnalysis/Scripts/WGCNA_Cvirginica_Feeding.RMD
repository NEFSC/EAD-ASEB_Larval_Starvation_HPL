...
Filt_abund_d0  <- read.csv(file="Output/Filtering_reads/D0.filtered_5cpm50perc.csv", sep=',', header=TRUE) 
dim(Filt_abund_d0) # 13096    13 - 13 columns and 38977 total genes 

Filt_abund_d1  <- read.csv(file="Output/Filtering_reads/D1.filtered_5cpm50perc.csv", sep=',', header=TRUE) 
dim(Filt_abund_d1) # 12556    13 - 13 columns and 38977 total genes 
...
dim(Filt_abund_matrix) # 12639    24 - 24 samples, gene IDs now row names

Filt_abund_matrix_d0 <- data.frame(Filt_abund_d0[,-1], row.names=Filt_abund_d0[,1]) 
dim(Filt_abund_matrix_d0) #  13096    12 - 12 samples, gene IDs now row names

Filt_abund_matrix_d1 <- data.frame(Filt_abund_d1[,-1], row.names=Filt_abund_d1[,1]) 
dim(Filt_abund_matrix_d1) # 12556    12 - 12 samples, gene IDs now row names
...
Metadata_all_raw <- Metadata %>% dplyr::filter(Sample.ID %in% colnames(t(dds_all_vst_t_trim)))
dim(Metadata_all_raw)
...
```{r}
postTOM_data = load(file = "Output/NetworkConstruction-postTOM.RData") # checkpoint upload to R
```
...
# 2235 2208 1587  918  850  751  661  596  531  517  493  483  452  274
...
table(dynamicColors_sign_d1) # lets look at this table...
      ...
      #   661        2208        1587         274         850         493         531 
      #  pink      purple         red      salmon         tan   turquoise      yellow 
      #   596         517         751         452         483        2235         918 
...
MEDiss_d1_merged = 1-cor(mergedMEs_d1); #plot the tree again
METree_d1_merged = hclust(as.dist(MEDiss_d1_merged), method = "average") # Cluster module eigengenes
plot(METree_d1_merged, 
     main = "Clustering of module eigengenes - Day 1 merged",
     xlab = "", sub = "")
# looks great! 
...
### All Phys 
```{r all phys as raw numeric form}

Metadata_all_raw
Metadata_all_raw$Treatment_asnum <- as.numeric(as.factor(Metadata_all_raw$Treatment))
Metadata_all_raw$Day_asnum  <- as.numeric(as.factor(Metadata_all_raw$Day))
Metadata_all_raw$Line_asnum <- as.numeric(as.factor(Metadata_all_raw$Line))

Metadata_D0_raw <- Metadata_all_raw %>% dplyr::filter(Day %in% 0)
Metadata_D1_raw <- Metadata_all_raw %>% dplyr::filter(Day %in% 1)

All.Phys.asnum             <- data.frame(lapply(Metadata_all_raw[,c(6:11,18:20)], function(x) as.numeric(as.character(x))),
                                         check.names=T, row.names = row.names(c(Metadata_all_raw)))
moduleTraitCor_Phys_all    <-  cor(MEs_all, All.Phys.asnum, use = "p");
moduleTraitPvalue_Phys_all <- corPvalueStudent(moduleTraitCor_Phys_all, nSamples);
moduleTraitPvalue_Phys_all 

MEs_d0_om <- MEs_d0[!(row.names(MEs_d0) %in% 'LSB_t3'),]

D0.Phys.asnum             <- data.frame(lapply(Metadata_D0_raw[,c(6:11,18,20)], function(x) as.numeric(as.character(x))),
                                         check.names=T, row.names = row.names(c(Metadata_D0_raw)))
moduleTraitCor_Phys_D0    <-  cor(MEs_d0_om, D0.Phys.asnum, use = "p");
moduleTraitPvalue_Phys_D0 <- corPvalueStudent(moduleTraitCor_Phys_D0, nSamples);
moduleTraitPvalue_Phys_D0 

# View(Metadata_D0_raw['Sample.ID'])
D1.Phys.asnum             <- data.frame(lapply(Metadata_D1_raw[,c(6:11,18,20)], function(x) as.numeric(as.character(x))),
                                         check.names=T, row.names = row.names(c(Metadata_D1_raw)))
moduleTraitCor_Phys_D1    <-  cor(MEs_d1, D1.Phys.asnum, use = "p");
moduleTraitPvalue_Phys_D1 <- corPvalueStudent(moduleTraitCor_Phys_D1, nSamples);
moduleTraitPvalue_Phys_D1 
...
## Shell length
```{r Shell length module trait relationships}
#=====================================================================================
#
...
## Respiration rate bins
```{r RR module trait relationships}

View(Metadata[,c('RR','RR_bins')])
Metadata_all_raw
#=====================================================================================
#
# Module trait correlations (Physiology bins!)
...
Metadata_all$RR_bins

# Shell Length ================= #
# all 
All_RR  <-  Metadata_all %>% dplyr::select('RR_bins') %>% # primary Line as Ambient (A) vs. Moderate (M)
  dplyr::mutate("9x10-5-4.2x10-4" = as.factor(as.numeric(RR_bins == 1)))  %>%
  dplyr::mutate("4.3x10-1x10-3" = as.factor(as.numeric(RR_bins == 2))) %>% 
  dplyr::mutate(">1x10-3" = as.factor(as.numeric(RR_bins == 3))) %>%
  dplyr::select(-RR_bins)
All.RR.asnum             <- data.frame(lapply(All_RR, function(x) as.numeric(as.character(x))),
                                         check.names=F, row.names = row.names(All_RR))
moduleTraitCor_RR_all    <-  cor(MEs_all, All.RR.asnum, use = "p");
moduleTraitPvalue_RR_all <- corPvalueStudent(moduleTraitCor_RR_all, nSamples);
moduleTraitPvalue_RR_all

# day 0 
d0_RR  <-  Metadata_D0 %>% dplyr::select('RR_bins') %>% # primary Line as Ambient (A) vs. Moderate (M)
  dplyr::mutate("9x10-5-4.2x10-4" = as.factor(as.numeric(RR_bins == 1)))  %>%
  dplyr::mutate("4.3x10-1x10-3" = as.factor(as.numeric(RR_bins == 2))) %>% 
  dplyr::mutate(">1x10-3" = as.factor(as.numeric(RR_bins == 3))) %>%
  dplyr::select(-RR_bins)
d0.RR.asnum             <- data.frame(lapply(d0_RR, function(x) as.numeric(as.character(x))),
                                         check.names=F, row.names = row.names(d0_RR))
moduleTraitCor_RR_d0    <-  cor(MEs_d0, d0.RR.asnum, use = "p");
moduleTraitPvalue_RR_d0 <- corPvalueStudent(moduleTraitCor_RR_d0, nSamples);
moduleTraitPvalue_RR_d0 # note we do not have all three bins present when separating by timepoint


# day 1 
d1_RR  <-  Metadata_D1 %>% dplyr::select('RR_bins') %>% # primary Line as Ambient (A) vs. Moderate (M)
  dplyr::mutate("9x10-5-4.2x10-4" = as.factor(as.numeric(RR_bins == 1)))  %>%
  dplyr::mutate("4.3x10-1x10-3" = as.factor(as.numeric(RR_bins == 2))) %>% 
  dplyr::mutate(">1x10-3" = as.factor(as.numeric(RR_bins == 3))) %>%
  dplyr::select(-RR_bins)
d1.RR.asnum             <- data.frame(lapply(d1_RR, function(x) as.numeric(as.character(x))),
                                         check.names=F, row.names = row.names(d1_RR))
moduleTraitCor_RR_d1    <-  cor(MEs_d1, d1.RR.asnum, use = "p");
moduleTraitPvalue_RR_d1 <- corPvalueStudent(moduleTraitCor_RR_d1, nSamples);
moduleTraitPvalue_RR_d1 # note we do not have all three bins present when separating by timepoint
```
...
# all 
All_Phys.matrix <-  paste(signif(moduleTraitCor_Phys_all, 2), "\n(",
                           signif(moduleTraitPvalue_Phys_all, 1), ")", sep = "")
par(mar = c(8, 9.5, 5, 3));
# png("Output/All/heatmaps/Groups_heatmap.png", 800, 1000, pointsize=20)
labeledHeatmap(Matrix = moduleTraitCor_Phys_all,
               xLabels = colnames(moduleTraitPvalue_Phys_all),
               yLabels = names(MEs_all),
               ySymbols = names(MEs_all),
               colorLabels = TRUE,
               colors = blueWhiteRed(50),
               textMatrix = All_Phys.matrix,
               setStdMargins = FALSE,
               cex.text = 1,
               zlim = c(-0.6,0.6),
               main = paste("All data: Module-trait relationships - GROUP"))
...
All.Phys.text <-  as.matrix(signif(moduleTraitPvalue_Phys_all, 3))
All.Phys.cor  <-  as.matrix(signif(moduleTraitCor_Phys_all, 3))
pa                  = cluster::pam(All.Phys.cor, k = 3)
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/All/heatmaps/AllPhys_heatmap.pdf", width=8, height=6)
Heatmap(moduleTraitCor_Phys_all, 
        name = "gene_cor", 
        rect_gp = gpar(col = "grey", lwd = 1),
        column_title = "All  WGCNA - group", 
        column_title_gp = gpar(fontsize = 12, fontface = "bold"),
        ...
        col = col_fun,
        cell_fun = function(j, i, x, y, width, height, fill) {
          grid.text(sprintf("%.1f", All.Phys.text[i, j]), x, y, gp = gpar(fontsize = 8))
        })
dev.off()


# day 0 
d0_Phys.matrix <-  paste(signif(moduleTraitCor_Phys_D0, 2), "\n(",
                           signif(moduleTraitPvalue_Phys_D0, 1), ")", sep = "")
par(mar = c(8, 9.5, 5, 3));
png("Output/Day0/heatmaps/D0_Phys_heatmap.png", 800, 1000, pointsize=20)
labeledHeatmap(Matrix = moduleTraitCor_Phys_D0,
               xLabels = colnames(moduleTraitPvalue_Phys_D0),
               yLabels = names(MEs_d0_om),
               ySymbols = names(MEs_d0_om),
               colorLabels = TRUE,
               colors = blueWhiteRed(50),
               textMatrix = d0_Group.matrix,
               setStdMargins = FALSE,
               cex.text = 1,
               zlim = c(-1.6,0.6),
               main = paste("d0 data: Module-trait relationships - Phys"))
dev.off()

d0.Treatment.text <-  as.matrix(signif(moduleTraitPvalue_Phys_D0, 3))
d0.Treatment.cor  <-  as.matrix(signif(moduleTraitCor_Phys_D0, 3))
pa                  = cluster::pam(d0.Treatment.cor, k = 2)
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/Day0/heatmaps/D0_Phys_heatmap.pdf", width=8, height=6)
Heatmap(moduleTraitCor_Phys_D0, 
        name = "gene_cor", 
        rect_gp = gpar(col = "grey", lwd = 1),
        column_title = "Day 0  WGCNA - group", 
        column_title_gp = gpar(fontsize = 12, fontface = "bold"),
...
# day 1
d1_Phys.matrix <-  paste(signif(moduleTraitCor_Phys_D1, 2), "\n(",
                           signif(moduleTraitPvalue_Phys_D1, 1), ")", sep = "")
par(mar = c(8, 9.5, 5, 3));
png("Output/Day0/heatmaps/Groups_heatmap.png", 800, 1000, pointsize=20)
labeledHeatmap(Matrix = moduleTraitCor_Phys_D1,
               xLabels = colnames(moduleTraitPvalue_Phys_D1),
               yLabels = names(MEs_d1),
               ySymbols = names(MEs_d1),
               colorLabels = TRUE,
               colors = blueWhiteRed(50),
               textMatrix = d1_Phys.matrix,
               setStdMargins = FALSE,
               cex.text = 1,
               zlim = c(-1.6,0.6),
               main = paste("d1 data: Module-trait relationships - Phys"))
dev.off()

d1.Treatment.text <-  as.matrix(signif(moduleTraitPvalue_Phys_D1, 3))
d1.Treatment.cor  <-  as.matrix(signif(moduleTraitCor_Phys_D1, 3))
pa                  = cluster::pam(d1.Treatment.cor, k = 2)
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/Day1/heatmaps/D1_Phys_heatmap.pdf", width=8, height=6)
Heatmap(moduleTraitCor_Phys_D1, 
        name = "gene_cor", 
        rect_gp = gpar(col = "grey", lwd = 1),
        column_title = "Day 1  WGCNA - group", 
        column_title_gp = gpar(fontsize = 12, fontface = "bold"),
...
# Assemble eigengne file - All data 
```{r}
...
# Assemble eigengne file - Day 0 data 
```{r}
...
# names (colors) of the modules
modNames_d0 = substring(names(MEs_d0), 3) # name all the modules, from 3rd character on (first two are ME)
geneModuleMembership_d0 = as.data.frame(cor(dds_D0_vst_t, MEs_d0, use = "p"));
MMPvalue_d0 = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_d0), nSamples));
names(geneModuleMembership_d0) = paste("MM", modNames_d0, sep="");
names(MMPvalue_d0) = paste("p.MM", modNames_d0, sep="");

D0_L_fed = as.data.frame(as.numeric(D0_Group$D0_L_fed)); # Define variable containing the desired column 
names(D0_L_fed) = "D0_L_fed"
D0_L_fed_geneTraitSignificance = as.data.frame(cor(dds_D0_vst_t, D0_L_fed, use = "p"));
D0_L_fed_GSPvalue = as.data.frame(corPvalueStudent(as.matrix(D0_L_fed_geneTraitSignificance), nSamples));
names(D0_L_fed_geneTraitSignificance) = paste("GS.", names(D0_L_fed), sep=""); # MA_geneTraitSignificance - pearsons correlation between reads and the MA grop
names(D0_L_fed_GSPvalue) = paste("p.GS.", names(D0_L_fed), sep=""); # corPvalueStudent
...
dim(annot) # 33777    5
names(annot) # view the column names to call 
probes = names(as.data.frame(dds_D0_vst_t)) # get the names of all genes in our dds dataframe 
probes2annot = match(probes, annot$Cvirginica_GeneID) # get the row numbers that overlap with the annotation file to map gene info
# The following is the number genes or 'probes' without annotation:
sum(is.na(probes2annot)) # 595 - these are LOC.. IDs that did not overlap!
# Should return 0.
...
# that was just mapped via 'match to the 'annot$Cvirginica_GeneID', thus the annot data merged based on row number in 'probes2annot'
# and we only need to call the data that is directly associated with dds_all_vst_t_trim, listed above 
geneInfo_GROUPS_d0 = data.frame(geneSymbol       = annot$Cvirginica_GeneID[probes2annot],# mapped with 'probes' to correct row numb
                             moduleColor      = moduleColors_d0, # associated with dds_all_vst_t_trim
                             KEGG_ID          = annot$Cgigas_KEGGID[probes2annot],# mapped with 'probes' to correct row numb
                             Protein_name     = annot$Cvirginica_Protein_name[probes2annot],# mapped with 'probes' to correct row numb
                             gene_length      = annot$Cvirginica_length[probes2annot],# mapped with 'probes' to correct row numb
                             GO.terms         = annot$Annotation_GO_ID[probes2annot],# mapped with 'probes' to correct row numb
                             D0_L_fed_geneTraitSignificance, D0_L_fed_GSPvalue) # associated with dds_all_vst_t_trim
# call this specific to the module and trait of interest
View(geneInfo_GROUPS_d0)
modOrder = order(-abs(cor(MEs_d0, D0_L_fed, use = "p"))); # order by the strength of the correlation between module and trait values for each sample

for (mod in 1:ncol(geneModuleMembership_d0)) { # Add module membership information 

  oldNames = names(geneInfo_GROUPS_d0)
  geneInfo_GROUPS_d0 = data.frame(geneInfo_GROUPS_d0, geneModuleMembership_d0[, modOrder[mod]], 
                               MMPvalue_d0[, modOrder[mod]]);
  names(geneInfo_GROUPS_d0) = c(oldNames, paste("MM.", modNames_d0[modOrder[mod]], sep=""),
                             paste("p.MM.", modNames_d0[modOrder[mod]], sep=""))
}
# Order the genes in the geneInfo variable first by module color, then by geneTraitSignificance
geneOrder_d0 = order(geneInfo_GROUPS_d0$moduleColor, -abs(geneInfo_GROUPS_d0$GS.D0_L_fed));
geneInfo_GROUPS_d0 = geneInfo_GROUPS_d0[geneOrder_d0, ]
View(geneInfo_GROUPS_d0)
...
write.csv(geneInfo_GROUPS_d0, file = "Output/Day0/Day0.WGCNA_ModulMembership.csv")
...
d1_data <- load(file="Output/Day1/Day1-NetworkConstruction_MEdata.RData")
d1_data
#=====================================================================================
#
# geneModuleMembership - geneTraitSignificance - GSPvalue
...
# names (colors) of the modules
modNames_d1 = substring(names(MEs_d1), 3) # name all the modules, from 3rd character on (first two are ME)
geneModuleMembership_d1 = as.data.frame(cor(dds_D1_vst_t, MEs_d1, use = "p"));
MMPvalue_d1 = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership_d1), nSamples));
names(geneModuleMembership_d1) = paste("MM", modNames_d1, sep="");
names(MMPvalue_d1) = paste("p.MM", modNames_d1, sep="");

d1_L_fed = as.data.frame(as.numeric(D1_Group$D1_L_fed)); # Define variable containing the desired column 
names(d1_L_fed) = "d1_L_fed"
d1_L_fed_geneTraitSignificance = as.data.frame(cor(dds_D1_vst_t, d1_L_fed, use = "p"));
d1_L_fed_GSPvalue = as.data.frame(corPvalueStudent(as.matrix(d1_L_fed_geneTraitSignificance), nSamples));
names(d1_L_fed_geneTraitSignificance) = paste("GS.", names(d1_L_fed), sep=""); # MA_geneTraitSignificance - pearsons correlation between reads and the MA grop
names(d1_L_fed_GSPvalue) = paste("p.GS.", names(d1_L_fed), sep=""); # corPvalueStudent
...
dim(annot) # 33777    5
names(annot) # view the column names to call 
probes = names(as.data.frame(dds_D1_vst_t)) # get the names of all genes in our dds dataframe 
probes2annot = match(probes, annot$Cvirginica_GeneID) # get the row numbers that overlap with the annotation file to map gene info
# The following is the number genes or 'probes' without annotation:
sum(is.na(probes2annot)) # 603 - these are LOC.. IDs that did not overlap!
# Should return 0.
...
#=====================================================================================
# Create the starting data frame
# d1_L_fed_geneTraitSignificance, d1_L_fed_GSPvalue, and moduleColors_all are asociated with dds_all_vst_t_trim
# that was just mapped via 'match to the 'annot$Cvirginica_GeneID', thus the annot data merged based on row number in 'probes2annot'
# and we only need to call the data that is directly associated with dds_all_vst_t_trim, listed above 
geneInfo_GROUPS_d1 = data.frame(geneSymbol       = annot$Cvirginica_GeneID[probes2annot],# mapped with 'probes' to correct row numb
                             moduleColor      = moduleColors_d1, # associated with dds_all_vst_t_trim
                             KEGG_ID          = annot$Cgigas_KEGGID[probes2annot],# mapped with 'probes' to correct row numb
                             Protein_name     = annot$Cvirginica_Protein_name[probes2annot],# mapped with 'probes' to correct row numb
                             gene_length      = annot$Cvirginica_length[probes2annot],# mapped with 'probes' to correct row numb
                             GO.terms         = annot$Annotation_GO_ID[probes2annot],# mapped with 'probes' to correct row numb
                             d1_L_fed_geneTraitSignificance, d1_L_fed_GSPvalue) # associated with dds_all_vst_t_trim
# call this specific to the module and trait of interest
View(geneInfo_GROUPS_d1)
modOrder = order(-abs(cor(MEs_d1, d1_L_fed, use = "p"))); # order by the strength of the correlation between module and trait values for each sample

for (mod in 1:ncol(geneModuleMembership_d1)) { # Add module membership information 

  oldNames = names(geneInfo_GROUPS_d1)
  geneInfo_GROUPS_d1 = data.frame(geneInfo_GROUPS_d1, geneModuleMembership_d1[, modOrder[mod]], 
                               MMPvalue_d1[, modOrder[mod]]);
  names(geneInfo_GROUPS_d1) = c(oldNames, paste("MM.", modNames_d1[modOrder[mod]], sep=""),
                             paste("p.MM.", modNames_d1[modOrder[mod]], sep=""))
}
# Order the genes in the geneInfo variable first by module color, then by geneTraitSignificance
geneOrder_d1 = order(geneInfo_GROUPS_d1$moduleColor, -abs(geneInfo_GROUPS_d1$GS.d1_L_fed));
geneInfo_GROUPS_d1 = geneInfo_GROUPS_d1[geneOrder_d1, ]
View(geneInfo_GROUPS_d1)
...
write.csv(geneInfo_GROUPS_d1, file = "Output/Day1/Day1.WGCNA_ModulMembership.csv")

```

# Module rlog Expression Plots
```{r}
#===================================================================================== 
# 
...
dim(dds.d0_rlogtrans) # 13096    12
dds.d0_rlogtrans <- dds.d0_rlogtrans[,rownames(t(dds_D0_vst))]
dim(dds.d0_rlogtrans) # 13096    12 - we  have the correct number of samples!
...
for(i in 1:nrow(modcolor)) {
  ...
  merged_Expdata_Mod <- merge(d0_rlog_Mod_MELT, Metadata, by ='Sample.ID')
  
  # mean Exp response table 
  meanEXp_Mod <- merged_Expdata_Mod %>% 
                  dplyr::select(c('Sample.ID','rlog_Expression','Treatment', 'Line', 'Group', 
                           'RR', 'ShellLength', 'AR_DOM', 'IR_C', 'IR_DOM')) %>% 
  ...
  RRplot     <- meanEXp_Mod %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=RR, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  Lengthplot <- meanEXp_Mod %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=ShellLength, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  ARDOMplot <- meanEXp_Mod %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=AR_DOM, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  IR_DOMplot <- meanEXp_Mod %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=IR_DOM, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  library(ggpubr)
  ggarrange(RRplot,Lengthplot, ARDOMplot, IR_DOMplot, ncol =2, nrow = 2)
...
# Module rlog Expression Plots - Day 1
```{r}
#===================================================================================== 
# 
# EXPLORE THE Expression of each module (for loop plots!) BY TREATMENT
...
Metadata_D1 # the treatment data from the dds object
dim(Metadata_D1) # 12  11


dds.d1_rlogtrans <- as.data.frame(rlogTransformation(assay(dds_D1))) # rlog transoform the expression data matrix (dds object)
dim(dds.d1_rlogtrans) # 12556    12
dds.d1_rlogtrans <- dds.d1_rlogtrans[,rownames(t(dds_D1_vst))]
dim(dds.d1_rlogtrans) # 12556    12 - we  have the correct number of samples!
dds.d1_rlogtrans <- tibble::rownames_to_column(dds.d1_rlogtrans,"TranscriptID") # rownames as first column

# write out this rlog tranformed master data (used for plotting below1) 
write.csv(dds.d1_rlogtrans, file = "Output/Day1/Day1_rlog_transformed.csv")

# call the module colors 
modcolor <- as.data.frame(unique(day1_ModuleMembership$moduleColor))
names(modcolor)[1] <- "color"
...
for(i in 1:nrow(modcolor)) {
  ...
  merged_Expdata_Mod_phys <- merge(d1_rlog_Mod_MELT, Metadata, by ='Sample.ID') # use to look at raw phys cors
 ...
 # mean Exp response table 
  meanEXp_Mod_phys <- merged_Expdata_Mod_phys %>% 
                  dplyr::select(c('Sample.ID','rlog_Expression','Treatment', 'Line', 'Group', 
                           'RR', 'ShellLength', 'AR_DOM', 'IR_C', 'IR_DOM')) %>% 
                  group_by(Sample.ID, Treatment, Line, Group, 
                           RR, ShellLength, AR_DOM, IR_C, IR_DOM) %>%
  ...
  RRplot     <- meanEXp_Mod_phys %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=RR, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  Lengthplot <- meanEXp_Mod_phys %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=ShellLength, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  ARDOMplot <- meanEXp_Mod_phys %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=AR_DOM, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  IR_DOMplot <- meanEXp_Mod_phys %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=IR_DOM, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
