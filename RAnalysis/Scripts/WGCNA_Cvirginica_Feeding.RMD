...
for(i in 1:nrow(modcolor)) {
  ...
  ARDOMplot <- meanEXp_Mod %>% 
                  ...
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  IR_Cplot <- meanEXp_Mod %>% 
                  mutate(Line_Treatment = paste0(Line, Treatment, sep='_')) %>% 
                  ggplot(aes(x=IR_C, y=mean.rlogExp, group = Line_Treatment)) + 
                  geom_line(aes(linetype =Treatment)) +
                  geom_point(aes(color = Line)) + 
                  theme_classic()
  library(ggpubr)
  ggarrange(RRplot,Lengthplot, ARDOMplot, IR_Cplot, ncol =2, nrow = 2)
